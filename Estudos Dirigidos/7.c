/* Alice da Silva de Lima 180112601
 * Problema do barbeiro dorminhoco.
 */ 
#include <pthread.h>
#include <semaphore.h>
#include <stdio.h>
#include <stdlib.h>
#include <unistd.h>

#define N_CLIENTES 50
#define N_CADEIRAS 5

sem_t sem_cadeiras;
sem_t sem_barbeiro;
sem_t cadeira;
sem_t atendimento;

void * f_barbeiro(void *v) {

  while(1) {
    sem_wait(&sem_barbeiro);      /*barbeiro dormindo*/
    
    sleep(1);                     /*cortando o cabelo do cliente*/
    printf("Barbeiro cortou o cabelo de um cliente\n"); 
    
    sem_post(&atendimento);       /*libera o cliente*/
  }
  pthread_exit(0);
}

void* f_cliente(void* v) {
  int id = *(int*) v;
  sleep(id%3);
  if(sem_trywait(&sem_cadeiras) == 0){    /*cliente pega uma cadeira de espera*/
     printf("Cliente %d entrou na barbearia \n",id);
     sem_wait(&cadeira);                  /*senta na cadeira do barbeiro*/
     sem_post(&sem_cadeiras);             /*libero uma cadeira de espera*/
     sem_post(&sem_barbeiro);             /*acorda o barbeiro*/
     sem_wait(&atendimento);              /*agurda o corte*/        
     sem_post(&cadeira);                  /*libera a cadeira do carteira*/
      printf("Cliente %d cortou o cabelo e foi embora \n",id);

  }else{//barbearia cheia
	printf("Barbearia cheia, cliente %d indo embora\n",id);
  }

  pthread_exit(0);
}

int main() {
  pthread_t thr_clientes[N_CLIENTES], thr_barbeiro;
  int i, id[N_CLIENTES];

  sem_init(&sem_cadeiras, 0, N_CADEIRAS);
  sem_init(&sem_barbeiro, 0, 0);
  sem_init(&cadeira, 0, 1);
  sem_init(&atendimento, 0, 1);
  
  for (i = 0; i < N_CLIENTES; i++) {
    id[i] = i;
    pthread_create(&thr_clientes[i], NULL, f_cliente, (void*) &id[i]);
  }

  pthread_create(&thr_barbeiro, NULL, f_barbeiro, NULL);
  
  for (i = 0; i < N_CLIENTES; i++) 
    pthread_join(thr_clientes[i], NULL);

  /* Barbeiro assassinado */
  
  return 0;
}

